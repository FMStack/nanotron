...
def tie_parameters(
    ...
    for tie_target, tie_model_ranks in ties:
        ...
        param, parent_module, param_name = get_parameter_and_parent_module(
            target=tie_target, root_module=root_module
        )
...
def create_pg_for_tied_weights(
    root_module: nn.Module, parallel_context: ParallelContext
):
    ...
    dist.all_gather_object(
        world_group_ranks, group_ranks, group=parallel_context.world_pg
    )
    ...
    for global_ranks in all_group_ranks:
        if global_ranks not in parallel_context.world_ranks_to_pg:
            parallel_context.world_ranks_to_pg[global_ranks] = dist.new_group(
                global_ranks
            )
...
def sync_tied_weights_gradients(
    ...
    for (name, group_ranks), tied_param in sorted(
        tied_id_to_param.items(), key=lambda x: x[0]
    ):
        ...
        if key in group_ranks_and_reduce_op_to_tensors_to_reduce:
            group_ranks_and_reduce_op_to_tensors_to_reduce[
                (group_ranks, tied_info.reduce_op)
            ].append(tied_grad)
        else:
            group_ranks_and_reduce_op_to_tensors_to_reduce[
                (group_ranks, tied_info.reduce_op)
    ...
    for (
        group_ranks,
        reduce_op,
    ), tensors in group_ranks_and_reduce_op_to_tensors_to_reduce.items():
