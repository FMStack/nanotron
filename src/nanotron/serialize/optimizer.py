...
def save_optimizer(
    ...
    """
    if (not optimizer.inherit_from(optim.ZeroDistributedOptimizer)) and dist.get_rank(
            ...
            if isinstance(optimizer, ZeroDistributedOptimizer):
                ...
                def convert_to_string(input_item):
                    if isinstance(input_item, dict):
                        ...
                        return tuple(
                            convert_to_string(element) for element in input_item
                        )
                    else:
                ...
                assert (
                    optimizer.param_name_to_dp_rank_offsets is not None
...
def load_optimizer(
    ...
    ckpt_expert_parallel_size = ckp_optimizer_config["parallelism"][
        "expert_parallel_size"
    ]

    if int(ckp_tp_size) != int(parallel_context.tp_pg.size()) or int(
        ckp_pp_size
    ) != int(parallel_context.pp_pg.size()):
        assert (
        ...
        def get_checkpoint_state_metadata(
            ...
            return param_shard_metadata[param_name.replace("module.", "")][
                (str(pp_rank), str(tp_rank))
            ]
        ...
        if ckp_optim_type == ZeroDistributedOptimizer.__name__:
            ...
            for shard_path in shard_paths:
                pp_rank, tp_rank = extract_parallel_ranks_from_shard_path(
                    shard_path, is_zero1=False
                )
        ...
        for param_index, param_name in tqdm(
            ...
            if not isinstance(param, NanotronParameter):
                raise NotImplementedError(
                    "Parameters are required to be NanotronParameter"
                )
            ...
            if param.is_sharded:
                ...
                for state_key in OPTIMIZER_STATE_NAMES:
                    ...
                    for (
                        pp_rank,
                        tp_rank,
                    ), ckp_optim_state in ckp_sharded_optim_states.items():
                        ...
                        ckp_shard_data = ckp_optim_state["state"][
                        ...
                        ckp_shard_metadata = get_checkpoint_state_metadata(
                            param_name, metadata_pp_rank, tp_rank
                        )
                        ...
                        if ckp_optim_type == ZeroDistributedOptimizer.__name__:
                            ...
                            orig_shape = ckp_optimizer_config["configs"][
                                "orig_param_shapes"
                            ][param_name]
                            orig_shape = [int(dim) for dim in orig_shape]
                            ckp_shard_data = ckp_shard_data.view(orig_shape)

                        new_optim_state_dict["state"][param_index][state_key] = (
                            merge_and_shard_tp_tensors(
                                ...
                                [
                                    (
                                        ckp_shard_data,
                                        ckp_shard_metadata.local_global_slices_pairs,
                                    ),
                                ],
                                new_shard_metadata,
                            )
                        ...
                        step = ckp_optim_state["state"][old_optim_state_index].get(
                            "step"
                        )
                        if step is not None:
    ...
    if isinstance(optimizer, ZeroDistributedOptimizer):
        ...
        if (
            int(ckp_tp_size) != parallel_context.tp_pg.size()
            or int(ckp_dp_size) != parallel_context.dp_pg.size()
        ):
            ...
            for param_index in state_dict["state"]:
                param_name = [
                    name
                    for idx, name in state_dict["names"].items()
                    if idx == param_index
                ][0]
                for state_name in OPTIMIZER_STATE_NAMES:
                    sliced_tensor = get_sliced_tensor(
                        param=state_dict["state"][param_index][state_name],
                        start_offset=optimizer.param_name_to_dp_rank_offsets[
                            param_name
                        ][current_dp_rank][0],
                        end_offset=optimizer.param_name_to_dp_rank_offsets[param_name][
                            current_dp_rank
                        ][1],
                    )
