...
@pytest.mark.skipif(
    available_gpus() < 2, reason="Testing test_ddp_with_afab requires at least 2 gpus"
)
...
def test_check_send_recv_tensor(send_contiguous: bool, full: bool):
    init_distributed(tp=1, dp=1, pp=2)(_test_check_send_recv_tensor)(
        send_contiguous=send_contiguous, full=full
    )


def _test_check_send_recv_tensor(
    parallel_context: ParallelContext, send_contiguous: bool, full: bool
):
    p2p = P2P(pg=parallel_context.pp_pg, device=torch.device("cuda"))
    if dist.get_rank(p2p.pg) == 0:
        tensor_to_send = torch.randn(
            3, 5, dtype=torch.float, device=torch.device("cuda")
        )
    ...
    if dist.get_rank(p2p.pg) == 0:
        ...
        torch.testing.assert_close(
            tensor_to_send, tensor_travelled_back_and_forth, atol=0, rtol=0
        )
    ...
    if full is False and send_contiguous is True:
        ...
        if dist.get_rank(p2p.pg) == 0:
            ...
            print(
                tensor_to_send.untyped_storage()[:4],
                tensor_travelled_back_and_forth.untyped_storage()[:4],
            )
            print(tensor_to_send.as_strided(size=(1,), stride=(1,), storage_offset=0))
            print(
                tensor_travelled_back_and_forth.as_strided(
                    size=(1,), stride=(1,), storage_offset=0
                )
            )
