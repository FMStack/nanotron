...
@pytest.mark.parametrize(
    "tokenizer_name_or_path", ["openai-community/gpt2", "unsloth/llama-3-8b-bnb-4bit"]
)
@rerun_if_address_is_in_use()
def test_build_nanoset_dataloader(
    ...
    json_paths, mmap_dataset_paths = create_dataset_paths(
    ...
    for idx, json_path in enumerate(json_paths):
        create_dummy_json_dataset(
            ...
            dummy_text=f"Nanoset {idx}!",
            n_samples=(idx + 1) * 50000,
        )
...
def _test_build_nanoset_dataloader(
    ...
    GLOBAL_BATCH_SIZE = (
        MICRO_BATCH_SIZE * N_MICRO_BATCHES_PER_BATCH * parallel_context.dp_pg.size()
    )

    # Preprocess dummy json datasets
    for json_path in json_paths:
        preprocess_dummy_dataset(
            path_to_json=json_path, tokenizer=tokenizer_name_or_path
        )
    ...
    for config in configs:
        ...
        dataset_sample_count = count_dataset_indexes(
            train_dataset.dataset_index, len(train_dataset.dataset_paths)
        )
        for idx, ds_length in enumerate(train_dataset.dataset_lengths):
            ...
            assert (
                np.max(
                    train_dataset.dataset_sample_index,
                    where=train_dataset.dataset_index == idx,
                    initial=-1,
                )
                < ds_length
            ...
            assert isclose(
                normalize(dataset_sample_count).tolist()[idx],
                train_dataset.dataset_weights[idx],
                abs_tol=0.05,
...
def _test_recover_nanoset_dataloader(
    ...
    for config in configs:
        ...
        for _ in range(
            skipped_batches + 1
        ):  # In order to compare with the first batch of the recovered DataLoader
        ...
        recovered_dataloader = build_nanoset_dataloader(
            ...
            consumed_train_samples=skipped_batches
            * MICRO_BATCH_SIZE
            * parallel_context.dp_pg.size(),
        )
